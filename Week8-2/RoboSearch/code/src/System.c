...
void goalTest(Building* pBuilding, bool verbose)
{
    ...
    // Goal Test1: from room N3.2 to charger room
...
{
    char sStartState[8]; // Declare a char array of sufficient size
    userInput(pBuilding);
    printf("Enter the initial state to start from (e.g. C3.2) : ");

    scanf("%7s", sStartState); // Read a string from the user into the array

    // Compare with the node's state in room list 
    // and return the room index of room list
    int startRoomIdx = getRoomIdx(pBuilding, sStartState); 
