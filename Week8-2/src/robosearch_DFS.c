/*
Depth-First Search (DFS) variant of RoboSearch
...
state queue[1000]   = {};   // This is the LIFO waiting queue - we put all states yet to be examined in here
...
int main() {
    ...
    printf("*** WELCOME TO ROBOSEARCH (DFS)***\n");
...
*
*  push and pop - control the queue operations that implement DFS as our search strategy
in contrast to before, DFS requires the LAST item in the queue to pe "popped"
...
// pop retrieves a state from the rear of the queue
state pop() {						
     if(stateCount > 0) {			// check if there are items in the queue
        //state s = queue[front];		// get state at front index - BFS ONLY
        //front++;				        // increase front index to point at the next state - BFS ONLY
        state s = queue[rear]; // DFS needs LIFO
        rear--;                // update queue pointer to point to the next state
